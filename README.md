...
Образовательный портал для подготовки к frontend-собеседованию. Модуль 1 — статьи с inline-упражнениями и test-режим. Модуль 2 — **Knowledge Journey**: обучающие маршруты с таймером, геймификацией и финальным отчётом.
...
Система, которая превращает тему в **активный опыт обучения**. Тема разбивается на чекпоинты — каждый содержит теорию и упражнения. Всё под давлением таймера, с XP, стриком и итоговым отчётом.

### Как попробовать

1. Запустить проект (`npm run dev`)
2. Открыть `http://localhost:3000/journey`
3. Выбрать одну из готовых тем
...
### Создание нового Journey

In-app генерация через OpenRouter временно отключена: кнопка «+ Новая тема» показывает заглушку после короткой имитации загрузки. Готовый системный промпт для ручной генерации лежит в `content/prompts/generate-journey.md`.
...
5. Убедиться, что поле `id` внутри JSON совпадает с желаемым URL: `/journey/<id>`
6. Перезапустить сервер, если он уже был запущен
...
src/lib/journey.ts                   ← автоматическая загрузка всех JSON из content/journeys
src/app/journey/page.tsx             ← Server Component (список готовых journey)
src/app/journey/JourneyClient.tsx    ← машина состояний (start → checkpoint → report)
src/components/journey/
  GenerateButton.tsx                 ← заглушка создания новой темы
...
В `content/journeys/` сейчас лежат два готовых маршрута. `src/lib/journey.ts` подхватывает оба автоматически, а `/journey/[id]` строит статические страницы по полю `id` внутри JSON.

| Файл                | URL                          | Тема                      | Чекпоинты | Упражнения |  XP |
| ------------------- | ---------------------------- | ------------------------- | --------: | ---------: | --: |
| `js-interview.json` | `/journey/js-interview-core` | JavaScript-собеседование  |         5 |         17 | 750 |
| `ts-typing.json`    | `/journey/ts-typing`         | TypeScript: система типов |         5 |         16 | 700 |
...
- **Next.js 16 (App Router)** — маршрутизация, SSG, Server Components
...
| `SpotTheBug`     | Найти строку с ошибкой в коде _(новый — Модуль 2)_ |
...
| `/journey`                    | Knowledge Journey _(Модуль 2)_ |
| `/journey/[id]`               | Прохождение готового Journey   |
...
| Критерий                                              | Статус                                                              |
| ----------------------------------------------------- | ------------------------------------------------------------------- |
| Система позволяет проходить journey                   | ✅ `/journey` + JSON-контент                                        |
| Генерация новых journey                               | ⚠️ in-app генерация отключена, доступен промпт для ручной генерации |
| Качество контента: вопросы осмысленные, разнообразные | ✅ 7 типов упражнений, промпт в `content/prompts/`                  |
...
Проект задеплоен на Vercel: https://result-curse-first-project.vercel.app/

Маршруты `/journey` и `/journey/[id]` доступны после деплоя текущего кода.
